...
namespace Appreciation.Manager.Services
{
    ...
    public class ServiceReadOnly<T> : BaseService, IServiceReadOnly<T> where T : BaseEntity
    {
        ...
        protected async Task<IEnumerable<T>> GetOrCreatePageAsync(string key, Func<int,int, string[], Task<IEnumerable<T>>> createItem,  int page, int pageSize, string[] arrays = null)
        {
            ...
            {
                // Key not in cache, so get data.
                cacheEntry = await createItem(page, pageSize, arrays);
        ...
        public async virtual Task<IEnumerable<T>> GetPageAsync(int page, int pageSize)
        {
            return await _repository.GetPageAsync(page, pageSize);
        }
    }
